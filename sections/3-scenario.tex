...
\revFix{We consider a Big Data platform whose pipeline lifecycle -- source-code development, artefact generation, deployment configuration, execution triggering, and version management, carried out by data engineers on behalf of multiple tenants -- is coordinated through an interface layer exposed as a REST API. Any Big Data platform exposes such a layer, through which pipeline operations are triggered and coordinated rather than executed directly against internal components, which is why its properties, not the specifics of any one platform, are what matter for security assurance. In our scenario, this layer takes the concrete form of a \emph{repository service} that allows for storing, versioning, publishing, and activating pipelines; pipeline code, deployment descriptors, build artefacts, and the orchestration are all managed through it.}
