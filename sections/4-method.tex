...
Building each oracle follows the same three-step procedure: the class of verification is analysed, the relevant failure conditions are identified, and the resulting criterion is encoded in the logic of the corresponding test.
What varies across tests is only the source from which the criterion is drawn, and that source is determined by the control being verified, not by the target.
