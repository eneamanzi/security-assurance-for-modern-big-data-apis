...
\revTtwo{The evaluation, conducted on a service instantiating the repository role of a Big Data pipeline delivery workflow and exposed through an API gateway,} answers the three research questions of Section~\ref{sec:intro}: the approach distinguishes correctly between conformance and violation on a testbed configured to yield both, including anomalies not anticipated in advance (RQ3); it surfaces a real misalignment between a published contract and the observed behaviour of the system, using criteria derived from the semantics of the security controls themselves (RQ1); and independent executions produce byte-identical results (RQ2), which makes the assessment comparable over time and embeddable in a continuous release cycle.
